...
pub fn get_all_characters(state: State<AppState>) -> Result<Vec<Character>, UserError> {
	...
	let merged_std_characters: Vec<Character> = preset_characters.into_iter().map(|mut preset| {
		...
		if let Some(saved) = std_repo_chars.iter().find(|c| c.get_id() == preset.get_id()) {
			// セリフだけを適用
	...
	// 自己修復
	state.character_repo.write()?.replace(result_characters.clone())?;
...
fn calculate_custom_frame_count() -> Result<usize, UserError> {
	...
	if is_addon_owned(AddonType::CustomFrameAdd1)? { allowed_frame_count += 1; }
	if is_addon_owned(AddonType::CustomFrameAdd2)? { allowed_frame_count += 2; }
	if is_addon_owned(AddonType::CustomFrameAdd4)? { allowed_frame_count += 4; }

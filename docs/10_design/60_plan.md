...
|2		|データ拡張パック未購入	|クリック	|プラン購入モーダルを表示(データ拡張)|
...
|2		|モーション拡張パック未購入	|クリック	|プラン購入モーダルを表示(モーション拡張)	|
...
|2		|キャラカスタム枠保持数が一定以下(<=n)	|クリック	|プラン購入モーダルを表示(キャラカスタム枠購入)	|
...
|---	|---	|---	|---	|
|1		|常時	|ホバー	|アンダーライン	|
|2		|常時	|クリック	|ストアページにリダイレクト(Steamoverlay) or 購入アイテムを指定して直接決済画面を表示する(Steam Microtransaction API)	|